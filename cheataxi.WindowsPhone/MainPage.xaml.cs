...
namespace cheataxi
{
    ...
    public sealed partial class MainPage : Page
    {
        ...
        BasicGeoposition location , sourseLocation, AimLocation;
        ...
        public MainPage()
        {
            ...
            {
                Fill = new SolidColorBrush(Windows.UI.Colors.DodgerBlue),
            ...
            pinAim.Children.Add(new Ellipse()
            {
            ...
            pinAim.Children.Add(new TextBlock()
            {
                Text = "Hello",
                FontSize = 12,
                Foreground = new SolidColorBrush(Windows.UI.Colors.White),
            ...
            try
            {
                ...
                while (ko < datalines.Length)
                {
                    ...
                    if (((start = datalines[ko-1].IndexOf("<pos>")) != -1) && (counter == selectedIndex))
                    {
                      ...
                      //  myMap.SetView(location = new BasicGeoposition() { Latitude = l2, Longitude = l1 });
                        if (!isSourseTapped)
                        {
                            myMap.Children.Clear();
                            //myMap.ShapeLayers.Clear();
                            myMap.Children.Add(pin);
                            MapControl.SetLocation(pin, new Geopoint(location = new BasicGeoposition() { Latitude = l2, Longitude = l1 }));
                            sourseLocation = location;
                            isSourseTapped = true;
                            tipLable.Text = "Введите или выберите на карте конечную точку маршрута:";
                            sourseLable.Visibility = Visibility.Visible;
